...
- **A peer learning backend** behind the Phase 0 Model/Trainer interface — same Environment and config as NEAT (Phase 4). Both backends are committed; more may be added as further phases.
...
- Single-env vs vectorised-env execution for PPO rollouts (NEAT evaluates genomes in isolation; PPO may still want several parallel envs for throughput).
...
- 2026-09-10 — Dropped "cross-game rules" from the shared-with-NEAT list and reworded the vectorised-env question to stop referencing NEAT's shared-space simulation, both following the platform scope cut (see `../OVERVIEW.md`): cross-game model transfer is no longer a goal and NEAT now evaluates genomes in isolation.
