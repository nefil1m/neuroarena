...
## Open questions
- Exact fields of `TrainingUpdate`, and how much per-generation detail is recorded always vs behind a verbosity setting — settled with Phase 7 / Phase 9.
- Whether `Objective.update` needs `terminated` / `truncated` passed explicitly or can read all it needs from `info`.
