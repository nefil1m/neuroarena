...
- **The interface must not assume "car-ness."** No raycasts, track, or steering in the base interface — those are car-game specifics that live behind it. It must fit future games too (Flappy Bird, a Mario-style platformer, Galaga, bullet hell — none implemented now, see Phase 11), which reuse the interfaces and infrastructure but are not expected to share trained models with the car game.
- **Model contract:** a pure function from observation vector to action vector. No game knowledge. A checkpoint records the observation- and action-space descriptors it was trained against.
- **Model / config compatibility check:** loading a checkpoint into an environment whose observation- or action-space descriptors differ from the checkpoint's is refused at run creation. This is resume-safety within one game — catching, for example, a sensor-layout change (Phase 3) that alters the observation shape — not a mechanism for running a model in a different game. No adapter layer, no canonical shared space.
...
- Exact fields of the local `Box` / `Discrete` descriptors, and whether the compatibility check compares dtype and bounds in addition to shape. This is the definition of the resume-compatibility check.
